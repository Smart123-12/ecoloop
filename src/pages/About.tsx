...
export const About: React.FC<AboutProps> = ({ setCurrentPage }) => {
  return (
    <div className="relative z-10 pt-24 pb-20 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
      ...
      <div className="absolute top-10 left-10 h-[300px] w-[300px] rounded-full bg-emerald-500/5 blur-[100px] pointer-events-none" />

      {/* Page Title */}
      <div className="text-left mb-12">
        <div className="flex items-center gap-2 mb-2">
          <HelpCircle className="h-5 w-5 text-emerald-400" />
          <span className="text-xs font-bold uppercase tracking-wider text-slate-400">Our Identity</span>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center mb-16 text-left">
        <div>
          ...
          <div className="flex gap-4 mt-6">
            <button 
              ...
              className="px-5 py-2.5 bg-gradient-to-r from-emerald-500 to-cyan-500 text-dark-900 font-bold rounded-lg text-xs shadow-md transition-all hover:scale-102"
            >
              Explore Trading Ledger
            </button>
            <button 
              onClick={() => setCurrentPage('contact')}
              className="px-5 py-2.5 border border-slate-700 bg-dark-800/40 text-slate-200 hover:text-white rounded-lg text-xs transition-all"
            >
              Request Whitepaper
            </button>
          </div>
        ...
        <div className="relative">
          ...
          <div className="relative rounded-xl border border-emerald-500/10 bg-slate-950 p-6 flex flex-col gap-4">
            {[
              ...
              <div key={idx} className="flex gap-4 p-4 rounded-lg bg-dark-900 border border-slate-800/50">
                <div className="h-9 w-9 rounded-full bg-emerald-500/10 flex items-center justify-center text-emerald-400 shrink-0 border border-emerald-500/10">
      ...
      <div className="text-center mb-10">
        <h2 className="text-xs font-bold uppercase tracking-widest text-emerald-400">Engineering</h2>
        <p className="mt-2 text-2xl font-extrabold text-white">EcoLoop Neural Platform Stack</p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        {[
          ...
          <GlowCard key={idx} className="flex flex-col gap-4">
            <span className="text-[10px] font-bold text-emerald-400 uppercase tracking-widest font-mono bg-emerald-950/40 px-2 py-0.5 rounded self-start border border-emerald-500/15">
              {tech.role}
            </span>
            <h3 className="text-base font-bold text-white mt-1">{tech.title}</h3>
            <p className="text-xs text-slate-400 leading-relaxed font-normal">{tech.desc}</p>
          </GlowCard>
        ))}
      </div>
    </div>
